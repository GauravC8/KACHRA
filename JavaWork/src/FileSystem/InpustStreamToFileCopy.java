 ...
 * 
 * try { InputStream is = new FileInputStream("write.txt"); OutputStream os =
 * new FileOutputStream("D:/"); byte[] buffer = new byte[1024]; int bytesRead;
 * while ((bytesRead = is.read(buffer)) != -1) { os.write(buffer, 0, bytesRead);
 * } System.out.println("File Copied"); is.close(); os.flush(); os.close(); }
